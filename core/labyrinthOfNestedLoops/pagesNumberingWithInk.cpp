...
int pagesNumberingWithInk1(int current, int numberOfDigits) {
    int x ;
    int digit = (log10(current) + 1);
    numberOfDigits-= digit;
    cout<<numberOfDigits<<endl;
    int treshold;
    while( numberOfDigits >= digit  ){
        x = ( numberOfDigits / digit );
        treshold = (pow(10, digit));
        if( current + x >= treshold ) {
            x = x - ((current + x ) - treshold );
            current += x;
            numberOfDigits -= ( x * digit );
        }else{
            current += x;
            numberOfDigits -= ( x * digit );
        }

         digit = (log10(current) + 1);
        cout<<x<<endl;
        cout<<treshold<<endl;
        cout<<current<<endl;
    }
...
int main(){
    cout<<pagesNumberingWithInk(8, 4);
}
